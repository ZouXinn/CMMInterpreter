...
MyLexer::MyLexer(const char* fileName)
{
	this->mySourceCode = readSourceCode(fileName);
	// try catch
	this->buildSymbols();
	try {
	...
	for (int i = 0; i < mySourceCode->length;)
	{
		...
		case '/': // / 或 // 或 /**/
			if (i + 1 < mySourceCode->length && this->mySourceCode->buffer[i + 1] == '/') // 单行注释
			{
				...
				{
					throw new Exception(LexerEx, row, "源程序应当以$符号结束");
				}
				else
				{
					row++;
			...
			else if (i + 1 < mySourceCode->length && this->mySourceCode->buffer[i + 1] == '*') // 多行注释
			{
				...
				while (true)
				{
					...
					{
						throw new Exception(LexerEx, row, "结束符号$不能在注释中");
					}
					else
					{
...
SourceCode* MyLexer::readSourceCode(const char* fileName)
{
	fstream in;
	...
	if (sc->length > 0)
	{
		...
		{
			throw Exception(LexerEx, -1, "没有结束符号");
		}
	}
	else
	{
